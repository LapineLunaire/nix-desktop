...
- camellya's nixpkgs instance is built with `cudaSupport`, which `obs-studio` reads to add `autoAddDriverRunpath` to its build
- Console and session keyboard layouts are Colemak with a plain US fallback, switched with `grp:win_space_toggle`
- Plasma wallpapers are read from the flake checkout, so the images stay out of the nix store
