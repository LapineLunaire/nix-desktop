...
in {
  services.pipewire.wireplumber.extraConfig."50-rodecaster"."monitor.alsa.rules" = [
    ...
    # pulseaudio clients list only nodes whose media.class is exactly Audio/Sink or Audio/Source, while wireplumber still links the /Internal variants to streams naming them in target.object, so the multichannel output keeps feeding the virtual sinks while applications see the sinks alone.
    # pro-input-1 stays a plain Audio/Source: it carries the main mix, which OBS captures directly.
    {
  ...
  services.pipewire.wireplumber.extraConfig."51-fiio-k11"."monitor.alsa.rules" = [
    ...
    # The K11 is driven by the RODECaster main mix loopback alone, so its output node is internal.
    {
